...
class EncoderDecoderSimple(nn.Module):
    def __init__(self):
        super().__init__()
...
class V2VModelSimple(nn.Module):
    def __init__(self, input_channels, output_channels):
        super().__init__()
        ...
        self.encoder_decoder = EncoderDecoderSimple()
...
if __name__ == '__main__':
    import time
    model = V2VModel(input_channels=32, output_channels=15)
    model = model.cuda()

    for i in range(10):
        input_tensor = torch.randn(8, 32, 64, 64, 64).cuda()
        start_time = time.time()
        output_tensor = model(input_tensor)
        end_time = time.time()
        print('time for one batch: {}'.format(end_time - start_time))
